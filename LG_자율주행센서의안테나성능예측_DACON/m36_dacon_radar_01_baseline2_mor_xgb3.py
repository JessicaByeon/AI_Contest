...
def xgb_hamsu(max_depth, min_child_weight, 
    ...
    params ={
        ...
        'subsample': max(min(subsample, 1), 0), # 0~1 사이로 정규화
    ...
    model = MultiOutputRegressor(XGBRegressor(**params)) # baysian_params 를 받아서 params 에 따라 변환하여 넣어줌
    
    # model = MultiOutputRegressor(XGBRegressor(tree_method='gpu_hist', predictor='gpu_predictor', gpu_id=0,
    #                                           random_state=123))
...
# 결과의 최대값 찾고
# {'target': 0.09690596036146759, 'params': {'colsample_bytree': 0.5, 'max_depth': 16.0, 'min_child_weight': 19.84370411e': 0.5, 'max_depth': 06722, 'reg_alpha': 0.01, 'reg_lambda': 0.001, 'subsample':01, 'reg_lambda': 0.00 1.0}}
...
# model = MultiOutputRegressor(XGBRegressor(
#     n_estimators = 500, learning_rate = 0.02,
#     max_depth = int(round(12.736597200858716)), # 반올림해서 정수형으로 변환 (무조건 정수형)
#     min_child_weight = int(round(24.500457733318566)),
#     subsample = max(min(0.7744869449803937, 1), 0), # 0~1 사이로 정규화
#     colsample_bytree = max(min(0.5, 1), 0),
#     reg_lambda = max(2.335855521956923, 0), # 무조건 양수만 받음
#     reg_alpha = max(3.1849165575058977, 0)
# ))
...
submit.to_csv(path + 'sample_submission_01_morxgb_03.csv', index=False)


# 01_morxgb_01 / n_iter=20 / bo random_state=1234
# {'target': 0.08867642427883715, 'params': {'colsample_bytree': 0.9257340088185118, 'max_depth': 15.299696938272689, 'min_child_weight': 10.775018130274647, 'reg_alpha': 23.839588553949426, 'reg_lambda': 8.226364868360802, 'subsample': 0.8977827579422104}}
# 0.09664949662233584

# 01_morxgb_02 / n_iter=20 -> 50 / bo random_state=1234 -> 123
# {'target': 0.09257790807659316, 'params': {'colsample_bytree': 0.8958288649190859, 'max_depth': 12.736597200858716, 'min_child_weight': 24.500457733318566, 'reg_alpha': 3.1849165575058977, 'reg_lambda': 2.335855521956923, 'subsample': 0.7744869449803937}}
# 0.09664949662233584

# 01_morxgb_03 / n_iter=20 -> 20 / bo random_state=1234 -> 123 / 파라미터 변경
# {'target': 0.09241406270916804, 'params': {'colsample_bytree': 0.943479734398287, 'max_depth': 13.882243388094382, 'min_child_weight': 22.81226570016747, 'reg_alpha': 3.0459121363849455, 'reg_lambda': 4.489696407902576, 'subsample': 0.8752629889402449}}
# 0.09163155595889173
