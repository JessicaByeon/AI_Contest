...
x_train, x_test, y_train, y_test = train_test_split(x, y,
        train_size=0.85, shuffle=True, random_state=123, stratify=y)

parameters = [
    {'RF__n_estimators': [100,200], 'RF__max_depth': [6,8,10,12], 'RF__min_samples_leaf': [3,5,7,10]},          # 32번
    {'RF__max_depth': [6,8,10,12], 'RF__min_samples_leaf': [3,5,7,10], 'RF__min_samples_split': [2,3,5,10]},    # 64번
    {'RF__min_samples_leaf': [3,5,7,10], 'RF__n_jobs': [-1,2,4]}                                            # 12번 = 총 108번
]

from sklearn.model_selection import KFold, StratifiedKFold
n_splits=5
kfold = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=123, #stratify=y
                        )
...
# # 6-1/ GridSearch
# # model.score :  0.8593350383631714
...
# submission.to_csv(path + 'sample_submission_03.csv', index=True)
...
# model.score :  0.8542199488491049
...
submission = pd.read_csv(path + 'sample_submission.csv', index_col=0)
submission['ProdTaken'] = y_summit
submission.to_csv(path + 'sample_submission_09.csv', index=True)
